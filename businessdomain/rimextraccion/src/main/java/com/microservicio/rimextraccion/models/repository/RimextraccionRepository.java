...
public interface RimextraccionRepository extends JpaRepository<TablaDinamica, Long> {
   ...
   @Query(value = "{CALL dbo.usp_Rim_Procedimiento_NuevaTabla(?)}", nativeQuery = true)
   ...
   @Query(value = "{CALL dbo.usp_Rim_Procedimiento_InsertarEnTablaDinamica(?, ?)}", nativeQuery = true)
